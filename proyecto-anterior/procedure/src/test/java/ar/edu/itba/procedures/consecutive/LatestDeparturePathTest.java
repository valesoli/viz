...
public class LatestDeparturePathTest extends FlightSmallDataSetTest {

    public LatestDeparturePathTest() {
        super(LatestDeparturePathProcedure.class);
    }
    ...
    private StatementResult callQuery(Session session) {
        return session.run(""
                ...
                + "-->()-->(:Value {value: 'C'})"
                + " CALL consecutive.latestDeparture(n, m, 1, {direction:\"outgoing\", edgesLabel: \"Flight\"})"
                + " YIELD path, interval"
