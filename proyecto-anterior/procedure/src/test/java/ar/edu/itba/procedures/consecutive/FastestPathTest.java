...
public class FastestPathTest extends FlightSmallDataSetTest {

    public FastestPathTest() {
        super(FastestPathProcedure.class);
    }
    ...
    private StatementResult callQuery(Session session) {
        return session.run(""
                ...
                + "-->()-->(:Value {value: 'B'})"
                + " CALL consecutive.fastest(n, m, 1, {direction:\"outgoing\", edgesLabel: \"Flight\"})"
                + " YIELD path, interval"
    ...
    void shouldReturnTheCorrectInterval() {
        try (Driver driver = GraphDatabase.driver(
            ...
            assertThat(result.stream())
                    .flatExtracting(r -> r.get("interval").asList())
                    .hasSize(1)
