...
public class SocialNetworkQueryDateIT extends IntegrationTest {

    public SocialNetworkQueryDateIT() {
        super("date.cypher");
    }
    ...
    @Test
    public void testCPathOfLength3SecondPath() {
        String compiledQuery = compileQuery(
                "SELECT p.path[0].attributes.Name as from, p.path[3].attributes.Name as to MATCH (n:Person),(n2:Person), " +
                        "p=cPath((n)-[:Friend*3]->(n2))\n" +
                        "SKIP 1\n" +
                        "LIMIT 1");
    ...
    public void testCPathOfLength3WithInterval() {
        String compiledQuery = compileQuery(
                "SELECT p.path[0].attributes.Name as from, p.path[3].attributes.Name as to, p.interval as interval\n" +
                        "MATCH (n:Person),(n2:Person),p = cPath((n)-[:Friend*3]->(n2), '2010-03-07', '2014-03-08')");
